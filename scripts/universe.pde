...
class Universe
{
  ...
  Universe(App app)
  {
    ...
    this.planets = new HashMap(256);
    this.sortedPlanets = null;
  }

  void setPopulationSize(int numPlanets)
  {
    // println("Current population size: " + this.planets.size());
    // println("Setting population size to: " + numPlanets);

    Planet planet = null;

    if (numPlanets > this.planets.size())
    {
      for (int i = this.planets.size(); i < numPlanets; ++i)
      {
        planet = this.pushPlanet();
      }
    }
    else if (numPlanets < this.planets.size())
    {
      for (int i = this.planets.size(); i > numPlanets; --i)
      {
      ...
      // println("New population size: " + this.planets.size());
    }
    else { return; }
  }
  ...
  void update()
  {
    ...
    pushMatrix();
    {
      ...
      Planet planet = null;
      Planet[] localPlanets = new Planet[this.planets.size()];

      // Update each planet & orbit
      // println("Num planets: " + this.planets.size());
      Iterator iterator = this.planets.entrySet().iterator();
      int i = 0;
      while (iterator.hasNext())
      {
        ...
        localPlanets[i++] = planet;
        // println("\tAdded planet ID: " + localPlanets[i-1].id);
      }

      // Sort the planets by depth
      if (javascript != null)
      {
        // println("Num planets to sort: " + localPlanets.length);
